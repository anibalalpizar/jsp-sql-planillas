...
public class beanPlanilla {
    ...
    public void changesPlanilla() throws SNMPExceptions, SQLException {
        Planilla pla = new Planilla(idPlanilla, descripcion, idTurno, idTipoPlanilla, fechaInicio, FechaFinal, fechaPago);
        PlanillaDB plaDB = new PlanillaDB();
        
        if (descripcion.equals("") || idTurno == 0 || idTipoPlanilla == 0 || fechaInicio.equals("") || FechaFinal.equals("")) {
            setMensaje("Llenar todos los campos para modificar la Planilla");
        } else {
            plaDB.ChangesPlanillas(pla);
            setMensaje("Planilla Modifcado Correctamente");
        }
